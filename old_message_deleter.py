...
handler = logging.RotatingFileHandler('discordOldMessageDeleter.log', maxBytes=1000000, backupCount=5)
...
async def prune(client, channel_id: int, delay_days: int):
    ...
    time_delta = datetime.timedelta(seconds=delay_days) if testing else datetime.timedelta(days=delay_days)
    async for msg in channel.history(limit=None,
                                     before=datetime.datetime.now() - time_delta):
        try:
            fire(msg.delete())
